...
//             return type === 'day' ? ' день назад' : type === 'hour' ? ' час назад' : ' минуту назад'
//         } else if (['2', '3', '4'].includes(time[time.length - 1])) {
//             return type === 'day' ? ' дня назад' : type === 'hour' ? ' часа назад' : ' минуты назад'
//         } else {
//             return type === 'day' ? ' дней назад' : type === 'hour' ? ' часов назад' : ' минут назад'
//         }
//     }

//     if (timeDifference / 60 / 24 > 24) {
...
export default function DateTimePrettyHocFunc(WrappedComponent, date) {
    ...
    class DateTimePretty extends React.Component {
      render() {
        return (
            <WrappedComponent date={prettyDate}/>
        );
      }  
    }
